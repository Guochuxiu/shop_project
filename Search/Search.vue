            ...
            <div class="py-container">
                ...
                <div class="bread">
                    ...
                    <ul class="fl sui-tag">
                        ...
                        <!-- 平台的售卖的属性值的展示 -->
                        <li class="with-x" v-for="(attr, index) in searchParams.props" :key="index" v-show="attr">
                                ...
                                <div class="list-wrap">
                                    ...
                                    <div class="attr">
                                        <a target="_blank" href="item.html"
                                            title="促销信息，下单即赠送三个月CIBN视频会员卡！【小米电视新品4A 58 火爆预约中】">{{ goods.goodsDesc }}</a>
                                    </div>
                                    <div class="commit">
                ...
                <Pagination :pageNo="8" :pageSize="3" :continue="5" :total="91" @changePage="changePage" />
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        getData() {
            console.log("接口调用", this.searchParams);
        ...
        trademarkInfo(trademark) {
            this.searchParams.trademark = `${trademark.id}:${trademark.name}`;
            this.getData();
        },
        removeTrademark() {
        ...
        changeOrder(flag) {
            ...
            let oringinOrder = this.searchParams.order.split(":")[1];
            let newOrde = '';
            if (flag == originFlag) {
                newOrde = `${originFlag}:${oringinOrder == 'desc' ? 'asc' : 'desc'}`
            } else {
                newOrde = `${flag}:desc`;
            }
            this.searchParams.order = newOrde;
            this.getData();
        },
        changePage(page) {
