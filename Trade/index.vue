  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address) in addressInfo" :key="address.id">
        <span class="username" :class="{ selected: address.isDefault == 1 }">{{ address.consignee }}
        </span>
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b><i>{{ orderInfo.totalNum || 0 }}</i>件商品，总商品金额</b>
          <span>¥{{ orderInfo.totalAmount || 0 }}.00</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ orderInfo.totalAmount || 0 }}.00</span>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      addressInfo: (state) => state.trade.addressList || [],  // 修复：address → addressList，加默认空数组
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      if (result.code == 200) {
        (this.orderId = result.data.orderNo || ''),
          //路由跳转 + 路由传递参数
