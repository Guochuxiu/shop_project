    ...
    <div class="register-container">
        ...
        <div class="register">
            ...
            <div class="content">
                ...
                <span class="error-msg">{{ errors.first('password') ? '请输入8-20位密码' : '' }}</span>
            </div>
            <div class="content">
                <label>确认密码:</label>
                <input placeholder="请输入确认密码" v-model="password1" name="password1" type="password"
                    v-validate="{ required: true, is: password }" :class="{ invalid: errors.has('password1') }" />
                <span class="error-msg">{{ errors.first('password1') ? '确认密码与密码不一致' : '' }}</span>
            </div>
            <div class="controls">
                <input type="checkbox" v-model="agree" name="agree" v-validate="{ required: true }"
    ...
    methods: {
        ...
        async register() {
            ...
            if (success) {
                ...
                let res = await this.$store.dispatch('register', { phone, code, password });
                if (res === "ok") {
                    alert('注册成功');
